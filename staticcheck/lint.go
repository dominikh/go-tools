		...
		if code.IsInTest(pass, fn) {
			...
			for i := 0; i < params.Len(); i++ {
				...
				if typeutil.IsType(param.Type(), "*testing.B") ||
					typeutil.IsType(param.Type(), "*testing.PB") ||
					typeutil.IsType(param.Type(), "testing.TB") {
		...
		for _, b := range fn.Blocks {
			for _, ins := range b.Instrs {
				...
				if _, ok := pure[callee.Object().(*types.Func)]; ok {
					...
					report.Report(pass, ins, fmt.Sprintf("calling %s only makes sense if its return values get used, but they are being ignored", callee.Name()))
				}
			}
		}
	}
