...
import (
	...
	"honnef.co/go/tools/internal/xtools-internal/expect"
...
func TestNoIndirectCreatePackage(t *testing.T) {
	...
	prog := ir.NewProgram(a.Fset, ir.SanityCheckFunctions|ir.PrintFunctions)
	...
	var got string
	for _, instr := range aSSA.Members["A"].(*ir.Function).Blocks[0].Instrs {
		if call, ok := instr.(*ir.Call); ok {
			f := call.Call.Value.(*ir.Function)
...
`,
	...
	for _, test := range tests {
		...
		irpkg, _, err := irutil.BuildPackage(&types.Config{Importer: importer.Default()}, fset,
			types.NewPackage("p", ""), []*ast.File{f}, ir.SanityCheckFunctions|ir.InstantiateGenerics)
		if err != nil {
			t.Errorf("test %q: %s", test.input[:15], err)
			continue
		}
...
`
	pkg, _ := buildPackage(t, input, ir.BuilderMode(0))
	...
	for fn := range irutil.AllFunctions(pkg.Prog) {
		if fn.Synthetic == "" {
			continue
		}
		name := fn.String()
...
`
	...
	p, _ := buildPackage(t, input, ir.BuilderMode(0))
	g := p.Func("g")
...
`
	...
	for _, mode := range []ir.BuilderMode{ir.BuilderMode(0), ir.InstantiateGenerics} {
		...
		for _, entry := range []struct {
			...
			t.Run(entry.name, func(t *testing.T) {
				...
				var wrapper *ir.Function
				for _, bb := range p.Func("init").Blocks {
					for _, i := range bb.Instrs {
						if store, ok := i.(*ir.Store); ok && v == store.Addr {
							switch val := store.Val.(type) {
							case *ir.Function:
								wrapper = val
							case *ir.MakeClosure:
								wrapper = val.Fn.(*ir.Function)
							}
						}
					}
				}
...
func TestTypeparamTest(t *testing.T) {
	...
	for _, p := range pkgs {
		...
		t.Run(originFilename, func(t *testing.T) {
			t.Parallel()
			prog, _ := irutil.Packages([]*packages.Package{p}, ir.SanityCheckFunctions|ir.InstantiateGenerics)
	...
	for _, item := range []struct {
		...
		t.Run(item.fn, func(t *testing.T) {
			...
			for _, b := range fn.Blocks {
				for _, instr := range b.Instrs {
					if call, ok := instr.(ir.CallInstruction); ok {
	...
	for _, mode := range []ir.BuilderMode{
		...
		for _, b := range p.Func("main").Blocks {
			for _, i := range b.Instrs {
				if call, ok := i.(ir.CallInstruction); ok {
...
func TestIssue58491(t *testing.T) {
	...
	conf := &types.Config{}
	p, _, err := irutil.BuildPackage(conf, fset, pkg, files, ir.SanityCheckFunctions|ir.InstantiateGenerics)
	if err != nil {
...
func TestSyntax(t *testing.T) {
	...
	p, _ := buildPackage(t, input, ir.InstantiateGenerics)
	prog := p.Prog
	...
	// go/ssa also tests that 'var _ = F[P]' does not add F[P] and its
	// anonymous functions because it's strictly speaking unreferenced. But in
	// go/ir, assignment to the blank identifier is visible in the IR and it
	// would be referenced.
	want := map[string]string{
	...
	if d := cmp.Diff(want, got); d != "" {
		t.Errorf("unexpected function signatures:\n%s", d)
	}
}
...
func TestLabels(t *testing.T) {
	...
	for _, test := range tests {
		buildPackage(t, test, ir.BuilderMode(0))
	}
}
...
func TestFixedBugs(t *testing.T) {
	for _, name := range []string{
		...
		t.Run(name, func(t *testing.T) {
			...
			mode := ir.SanityCheckFunctions | ir.InstantiateGenerics
			// mode |= ir.PrintFunctions // debug mode
			if _, _, err := irutil.BuildPackage(&types.Config{}, fset, pkg, files, mode); err != nil {
				t.Error(err)
			}
		})
	}
...
func TestIssue67079(t *testing.T) {
	...
	spkg, ppkg := buildPackage(t, src, ir.BuilderMode(0))
	prog := spkg.Prog
...
`
	...
	p, _ := buildPackage(t, source, ir.InstantiateGenerics)

	probes := callsTo(irutil.AllFunctions(p.Prog), "print")
...
func constString(v ir.Value) string {
...
func TestMultipleGoversions(t *testing.T) {
	...
	p, _, err := irutil.BuildPackage(conf, fset, pkg, files, ir.SanityCheckFunctions)
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, test := range []struct {
		global *ir.Global
		want   string // basic block to []*ir.Alloc.
	}{
		...
		for _, b := range p.Func("init").Blocks {
			for _, instr := range b.Instrs {
				if s, ok := instr.(*ir.Store); ok && s.Addr == test.global {
					fn, _ = s.Val.(*ir.Function)
				}
			}
		}
		if fn == nil {
			t.Fatalf("Failed to find *ir.Function for initial value of global %s", test.global)
		}
...
func TestRangeOverInt(t *testing.T) {
	...
	pkg := types.NewPackage("p", "")
	conf := &types.Config{}
	p, _, err := irutil.BuildPackage(conf, fset, pkg, []*ast.File{f}, ir.SanityCheckFunctions)
	if err != nil {
		t.Fatal(err)
	}
